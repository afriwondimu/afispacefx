import React from 'react'

type Props = {}

const Footer = (props: Props) => {
  return (
    <div className='bg-[#161b22]'>
    <div className='footer relative pt-14 break-words '>
      <div className='max-w-[1280px] mx-auto relative z-[2] overflow-hidden'>
        <div className='flex flex-col lg:flex-row py-10 mb-8 space-x-6 px-4'>
            <div className='mb-12 px-2'>
            <a className="" href="https://github.com">
              <svg
                height="40.000000pt"
                aria-hidden="true"
                fill="currentColor"
                viewBox="0 0 900.000000 440.000000"
                version="1.1"
                width="75.000000pt"
                data-view-component="true"
                className=" text-white"
              >
                <g
                  transform="translate(0.000000,485.000000) scale(0.100000,-0.100000)"
                  fill="#ffffff"
                  stroke="none"
                >
                  <path
                    d="M3175 3799 c-19 -17 -35 -36 -35 -41 0 -6 -13 -27 -28 -47 -16 -20
-32 -43 -36 -51 -11 -23 -24 -43 -131 -205 -55 -82 -102 -154 -104 -160 -2 -5
-17 -26 -32 -46 -16 -20 -29 -40 -29 -43 0 -4 -8 -19 -19 -34 -10 -15 -57 -87
-105 -159 -47 -73 -125 -193 -173 -265 -92 -139 -108 -163 -119 -188 -4 -8
-20 -31 -36 -51 -15 -20 -28 -40 -28 -44 0 -4 -13 -24 -30 -45 -16 -21 -30
-41 -30 -46 0 -4 -6 -15 -13 -23 -24 -28 -40 -80 -33 -106 13 -50 57 -65 198
-65 136 0 162 5 200 42 27 26 128 175 154 228 4 8 27 43 51 77 24 35 43 65 43
68 0 2 13 24 29 47 42 62 145 221 151 233 3 6 36 58 75 115 76 114 89 135 104
165 34 65 149 220 163 220 10 0 46 -44 92 -114 42 -63 76 -116 76 -118 0 -2 8
-16 19 -31 10 -15 43 -65 73 -112 30 -47 81 -125 114 -175 32 -49 61 -95 64
-100 3 -6 26 -42 51 -80 65 -98 123 -188 129 -200 24 -47 147 -219 172 -240
31 -26 31 -26 180 -23 l150 3 24 28 c41 47 35 65 -71 236 -6 9 -42 63 -80 121
-39 58 -72 110 -75 115 -6 12 -111 174 -151 233 -16 23 -29 45 -29 48 0 4 -13
23 -29 43 -15 20 -30 41 -32 46 -4 10 -153 239 -210 323 -16 23 -29 45 -29 48
0 4 -13 23 -30 44 -16 21 -30 41 -30 44 0 3 -20 35 -45 72 -25 36 -45 67 -45
69 0 10 -95 141 -114 157 -18 15 -42 17 -173 18 l-152 0 -36 -31z"
                  />
                  <path
                    d="M4722 3800 c-20 -27 -22 -41 -22 -168 0 -83 4 -142 11 -149 14 -14
1803 -6 1842 9 36 14 42 39 42 175 0 104 -2 113 -24 137 l-24 26 -902 0 -901
0 -22 -30z"
                  />
                  <path
                    d="M6912 3807 l-22 -23 2 -779 3 -779 34 -23 c30 -21 46 -23 148 -23
124 0 157 10 187 54 14 23 16 95 16 774 l0 749 -23 34 -23 34 -150 3 c-146 2
-151 2 -172 -21z"
                  />
                  <path
                    d="M4706 3204 c-3 -9 -6 -227 -6 -486 l0 -470 31 -29 31 -29 138 0 c132
0 138 1 164 25 l26 24 0 278 c1 248 3 280 18 298 38 45 39 45 720 45 406 0
661 4 683 10 50 14 69 62 69 174 0 97 -17 147 -55 164 -19 9 -250 12 -919 12
-806 0 -894 -2 -900 -16z"
                  />
                </g>
              </svg>
            </a>
              <div className='text-white'>
                <h2 className='mt-6 font-semibold'>Learn, share, and grow</h2>
                <p className="text-[14px] text-[#e9e9e9] mb-6 ">Empower Your Trading Journey with AFI Space FX</p>
                
              </div>
            </div>
 
        </div>
      </div>
      
        <div className='max-w-[1280px] mx-auto text-[12px] md:flex flex-row-reverse py-6 justify-between items-center px-4'>
          <ul className='flex items-center max-md:mb-4  '>
            <li className='mr-4'>
              <a href="https://x.com/AFIspaceFX?t=UHEf1cc9kmfB9X0uIlPrwA&s=09"><img src="https://raw.githubusercontent.com/afriwondimu/assets/c0d032416404462515f2a786155aa38206f7d353/assets/icons8-twitterx%20(4).svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href="https://t.me/AFISpaceFx"><img src="https://raw.githubusercontent.com/afriwondimu/assets/0cbebbf6fca3837c4dbe1ff480e32609c4c74a0b/assets/icons8-telegram.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href="https://www.instagram.com/afispacefx?igsh=a3d4bzZiZ2xja3ky"><img src="https://raw.githubusercontent.com/afriwondimu/assets/953340444fecc739ceda13567346d6f0b3404fcd/assets/icons8-instagram-640.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href="https://discord.gg/47ENshX6Vt"><img src="https://raw.githubusercontent.com/afriwondimu/assets/ba85c28935aeecbc9f652ed79a16d5e18f59d5bc/assets/icons8-discord.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href=""><img src="https://raw.githubusercontent.com/afriwondimu/assets/1a2bb2959171d307cfc3dadef175c75aa71fec69/assets/icons8-facebook.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href="https://youtube.com/@AFISpaceFx?si=heDqidHzmzBV-x6U"><img src="https://raw.githubusercontent.com/afriwondimu/assets/41afa3fc74a8eb9ff82aa90f68a95a79f82021d1/assets/icons8-youtube.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
            <li className='mr-4'>
              <a href="https://tiktok.com/@AFISpaceFX"><img src="https://raw.githubusercontent.com/afriwondimu/assets/8757b78d2c961f357c7d91da7e235e79e071f73d/assets/icons8-tik-tok.svg" height="18" width="22" className="d-block" loading="lazy" decoding="async" alt="Twitter icon" /></a>
            </li>
          </ul>
          <ul className='flex items-center mb-4 sm:mb-0 text-[#7d8590] flex-wrap' >
            <li className="mr-3 ">© 2024 AFI, All rights reserved.</li>
          </ul>
        </div>
      </div>
      </div>

  )
}

export default Footer